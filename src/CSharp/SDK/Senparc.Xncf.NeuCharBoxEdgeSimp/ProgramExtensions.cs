...
namespace Senparc.Xncf.NeuCharBoxEdgeSimp
{
    ...
    {
        public static void AddNcf(this WebApplicationBuilder builder, IXncfRegister mainRegister)
        {
            //builder.Services.Configure<SenderReceiverSet>(builder.Configuration.GetSection(SenderReceiverSet.SenderReceiverSetKey));
            SenderReceiverSet senderReceiverSet = new SenderReceiverSet();
            builder.Configuration.Bind(SenderReceiverSet.SenderReceiverSetKey, senderReceiverSet);
            senderReceiverSet.XncfUId = mainRegister.Uid;
            builder.Services.AddSingleton(senderReceiverSet); // 注入为可变对象

            Helper.CertHepler.SetSenderReceiverSet(senderReceiverSet);//设置SenderReceiverSet对象

            builder.Services.AddScoped<Respository.ISysButtonRespository, Respository.SysButtonRespository>();
            builder.Services.AddScoped<Respository.ISysRolePermissionRepository, Respository.SysRolePermissionRepository>();
            builder.Services.AddScoped<INcfDbData>(provider => null);

            //激活 Xncf 扩展引擎（必须）
            //var logMsg = builder.StartWebEngine(new[] { "Senparc.Areas.Admin" });
            var logMsg = builder.StartWebEngine(new[] { "" });
            ...
            //builder.Services.AddHostedService<EdgeBackgroundService>();
#if !DEBUG
            ...
            builder.Services
                .AddMcpServer()
                .WithHttpTransport()
                .WithToolsFromAssembly(mainRegister.GetType().Assembly);//抓取mainRegister所在的程序集
